...
class ParseVisitor(ast.NodeVisitor):
    ...
    def visit_BinOp(self, node):
        type_pair = self.make_type_pair(node.left, node.right)
        if type_pair is not None:
            self.type_hint_pair += [type_pair]
        self.generic_visit(node)
        return node
...
class ReturnStmtVisitor(ast.NodeVisitor):
    ...
    def backward(self, cfg, block, return_value):
        ...
        if type_val in ["ID", "attr"]:
            lookup_name = init_val.id if type_val == "ID" else get_attr_name(init_val)
            
            if lookup_name in self.local_assign_records: 
                right = self.local_assign_records[lookup_name][-1]
                self.type_infer(right)
            elif lookup_name[0:5] == "self." and lookup_name[5:] in self.class_assign_records:
    ...
    def type_infer_CFG(self, node):
        # function def node as the input
        #return X.mean()
        # cfg for this fun node
        # convert function def node to a module node
        ...
        #if node.name == "to_native_string":
        #    #cfg.build_visual('res/backward-example', 'pdf') 
    ...
    def type_infer(self, node):
        ...
        if type_val == 'ID':
          ...
          if first_part_name in self.args or first_part_name in self.class_assign_records["init_arg_name_lst"]:
            ...
            if hasattr(node, "value") and isinstance(node.value, ast.Name) and node.value.id in self.args:
                self.r_types += ['input']
        else:
           # known type
            self.r_types += [type_val]
